...
> **Preview software:** `v2.0.0-preview.9` is suitable for testing and contribution, but profiles should still be backed up before upgrades. Calculations marked **strategic** or **heuristic** are not claimed to be exact game formulas.
...
Output: **`dist\TowerOptimizer-2.0.0-preview.9-Windows.zip`** — ship that single file. Optional single-exe build: `.\scripts\build_windows_exe.ps1 -SingleExe`. See [Portable Windows executable](docs/WINDOWS_EXE.md).
