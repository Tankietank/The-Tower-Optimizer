...
**`TowerOptimizer-2.0.0-preview.6-Windows.zip`** (name may vary by version)
...
2. Wait for **“Starting Tower Optimizer…”** (up to 30 seconds)
...
2. Double-click it
3. First launch may take **30–60 seconds** while it unpacks internally
...
| Problem | Fix |
| --- | --- |
| Nothing happens after double-click | Wait 30 seconds. Check **`launcher.log`** in `%LOCALAPPDATA%\TowerOptimizer` (or `data\launcher.log` in portable mode). |
